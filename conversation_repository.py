...
def add_private_message(id, content):
    sql = "INSERT INTO private_messages (user1_id, user2_id, content, " \
        "timestamp, visible, seen) " \
...
def get_private_messages_info():
    sql = "SELECT p.user1_id as contact_id, MAX(p.timestamp) as viimeisin, " \
        "COUNT(*) as count, " \
        "(SELECT u.name FROM users u WHERE p.user1_id=u.id) as name, " \
        "(SELECT COUNT(*) FROM private_messages p2 " \
        "WHERE p2.user1_id=p.user1_id AND p2.user2_id=:user_id AND p2.seen=False) " \
        "as not_seen " \
...
def search_private_messages(query):
    sql = "SELECT p.id as message_id, p.content as content, p.user1_id " \
        "as sender_id, u.name as username, p.timestamp as timestamp " \
        "FROM private_messages p " \
        "LEFT JOIN users u ON u.id=p.user1_id " \
        "WHERE p.content LIKE :query AND p.user2_id=:user_id AND p.visible=True " \
        "ORDER BY timestamp DESC LIMIT 30;"
    return db.session.execute(sql, {"query":query,
